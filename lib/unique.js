...
function mergeCommands(commands1, commands2) {
  if (commands1.delete && commands2.insert && commands1.delete.id === commands2.insert.id) {
      ...
      //commands1 did a delete, so if commands2 does a delete, it assumes that
      //commands1 has already performed its delete. So we need
      //to make a slight adjustment
      if (commands2.delete && commands2.delete.id >= commands1.update.id) {
        commands2.delete.id++;
...
Unique.prototype.trigger = function (obj) {
  var events = ['delete', 'update', 'insert'];
  var event, command;
  for (var i = 0; i < 3; i++) {
    event = events[i];
    command = obj[event];
...
Unique.prototype.handleDelete = function (option) {
  var commands, oldSerIndex;

  oldSerIndex = this.getSerIndexForOptionId(option.id);

  //remove this entry from the existing ser
  commands = this.removeFromExistingSer(oldSerIndex, option);
